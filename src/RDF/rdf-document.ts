...
export class RdfDocument {
    ...
    constructor(public uri: string, dataset?: DatasetCore, prefixes?: Prefixes){
        
    }

    static async getRdfDocument(uri: string, fetch: Fetch): Promise<RdfDocument>{
        const result: ParserResult = await fetch(uri).then(res => res.text()).then(res => parseTurtle(res, uri))
